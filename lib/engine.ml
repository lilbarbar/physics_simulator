...
let create ~height ~width ~initial_snake_length =
  ...
  | Some apple ->
    ...
    if
      List.exists (Snake.all_positions snake) ~f:(fun pos ->
...
let check_for_collisions t =
  ...
  <- (if not (Board.in_bounds t.board (Snake.head t.snake))
      then Game_state.Game_over "Out of bounds!"
      else Game_state.In_progress)
;;
...
let maybe_consume_apple t =
  ...
  let apple = Apple.position t.apple in
  if Position.equal apple snake_head
  then (Snake.grow_over_next_steps t.snake (Apple.amount_to_grow t.apple);
